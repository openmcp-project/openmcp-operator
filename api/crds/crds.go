...
func CRDs() ([]*apiextv1.CustomResourceDefinition, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error reading CRD directory: %w", err)
	}
	res := []*apiextv1.CustomResourceDefinition{}
	for _, f := range files {
		data, err := CRDFS.ReadFile(filepath.Join("manifests", f.Name()))
		if err != nil {
			return nil, fmt.Errorf("error reading CRD file '%s': %w", f.Name(), err)
		}
		crd := &apiextv1.CustomResourceDefinition{}
		if err := yaml.Unmarshal(data, crd); err != nil {
			return nil, fmt.Errorf("error parsing file '%s' into CRD: %w", f.Name(), err)
		}
		res = append(res, crd)
	}
	return res, nil
