...
func (o *RunOptions) Run(ctx context.Context) error {
	...
	mgr, err := ctrl.NewManager(o.PlatformCluster.RESTConfig(), ctrl.Options{
		...
		LeaderElectionID:        "github.com/openmcp-project/openmcp-operator--helm-deployer",
	...
	// setup helm deployer
	helmRec := helm.NewHelmDeploymentController(o.PlatformCluster, mgr.GetEventRecorder(helm.ControllerName), o.ProviderName, providerSystemNamespace, o.Environment)
	if err := helmRec.SetupWithManager(mgr); err != nil {
		return fmt.Errorf("unable to setup helm deployer with manager: %w", err)
	}
