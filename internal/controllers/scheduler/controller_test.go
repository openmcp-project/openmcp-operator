...
const (
	platform      = "platform"
	schedulerRec  = "scheduler"
	pSchedulerRec = "preemptiveScheduler"
)
...
func defaultTestSetup(testDirPathSegments ...string) (*config.SchedulerConfig, *testutils.ComplexEnvironment) {
	...
	env := testutils.NewComplexEnvironmentBuilder().
		...
		WithInitObjectPath(platform, append(testDirPathSegments, "cluster")...).
		WithReconcilerConstructor(schedulerRec, func(c ...client.Client) reconcile.Reconciler {
			r, err := scheduler.NewClusterScheduler(nil, clusters.NewTestClusterFromClient(platform, c[0]), cfg.Scheduler)
			Expect(err).ToNot(HaveOccurred())
			return r
		}, platform).
...
var _ = Describe("Scheduler", func() {
	...
	Context("Scope: Namespaced", func() {
		...
		It("should create a new exclusive cluster if no cluster exists", func() {
			...
			Expect(env.Client(platform).Get(env.Ctx, ctrlutils.ObjectKey(exclusiveString, "foo"), req)).To(Succeed())
			Expect(req.Status.Cluster).To(BeNil())

			env.ShouldReconcile(schedulerRec, testutils.RequestFromObject(req))
		...
		It("should create a new shared cluster if no cluster exists", func() {
			...
			req := &clustersv1alpha1.ClusterRequest{}
			Expect(env.Client(platform).Get(env.Ctx, ctrlutils.ObjectKey("shared", "foo"), req)).To(Succeed())
			Expect(req.Status.Cluster).To(BeNil())
		...
		It("should share a shared cluster if it still has capacity and create a new one otherwise", func() {
			...
			Expect(env.Client(platform).Get(env.Ctx, ctrlutils.ObjectKey("shared2", "foo"), req2)).To(Succeed())
			...
			Expect(env.Client(platform).Get(env.Ctx, ctrlutils.ObjectKey("shared3", "foo"), req3)).To(Succeed())
			Expect(req3.Status.Cluster).To(BeNil())

			env.ShouldReconcile(schedulerRec, testutils.RequestFromObject(req3))
		...
		It("should take over annotations and labels from the cluster template", func() {
			...
			Expect(env.Client(platform).Get(env.Ctx, ctrlutils.ObjectKey(req.Status.Cluster.Name, req.Status.Cluster.Namespace), cluster)).To(Succeed())
		...
		It("should use the request's namespace if none is specified in the template and ignore clusters that don't match the label selector", func() {
			...
			Expect(env.Client(platform).List(env.Ctx, fooClusters, client.InNamespace(clusterNamespace))).To(Succeed())
			...
			req2 := &clustersv1alpha1.ClusterRequest{}
			Expect(env.Client(platform).Get(env.Ctx, ctrlutils.ObjectKey("shared", "bar"), req2)).To(Succeed())
			Expect(req2.Status.Cluster).To(BeNil())
			barClusters := &clustersv1alpha1.ClusterList{}
			Expect(env.Client(platform).List(env.Ctx, barClusters, client.InNamespace("bar"))).To(Succeed())
			...
			req3 := &clustersv1alpha1.ClusterRequest{}
			Expect(env.Client(platform).Get(env.Ctx, ctrlutils.ObjectKey("shared2", "foo"), req3)).To(Succeed())
			Expect(req3.Status.Cluster).To(BeNil())
			env.ShouldReconcile(schedulerRec, testutils.RequestFromObject(req3))
			Expect(env.Client(platform).Get(env.Ctx, client.ObjectKeyFromObject(req3), req3)).To(Succeed())
			Expect(req3.Status.Cluster).ToNot(BeNil())
	...
	Context("Scope: Cluster", func() {
		...
		It("should evaluate all namespaces in cluster scope", func() {
			...
			req2 := &clustersv1alpha1.ClusterRequest{}
			Expect(env.Client(platform).Get(env.Ctx, ctrlutils.ObjectKey("shared2", "bar"), req2)).To(Succeed())
			Expect(req2.Status.Cluster).To(BeNil())
			env.ShouldReconcile(schedulerRec, testutils.RequestFromObject(req2))
			Expect(env.Client(platform).Get(env.Ctx, client.ObjectKeyFromObject(req2), req2)).To(Succeed())
			Expect(req2.Status.Cluster).ToNot(BeNil())
			...
			// this should re-use the existing cluster
			req3 := &clustersv1alpha1.ClusterRequest{}
			Expect(env.Client(platform).Get(env.Ctx, ctrlutils.ObjectKey("shared3", "baz"), req3)).To(Succeed())
			Expect(req3.Status.Cluster).To(BeNil())
			env.ShouldReconcile(schedulerRec, testutils.RequestFromObject(req3))
			Expect(env.Client(platform).Get(env.Ctx, client.ObjectKeyFromObject(req3), req3)).To(Succeed())
			Expect(req3.Status.Cluster).ToNot(BeNil())
	...
	It("should handle the delete-without-requests label correctly", func() {
		...
		// should create a new cluster
		req := &clustersv1alpha1.ClusterRequest{}
		Expect(env.Client(platform).Get(env.Ctx, ctrlutils.ObjectKey("delete", "foo"), req)).To(Succeed())
		Expect(req.Status.Cluster).To(BeNil())
		...
		Eventually(func() bool {
			return apierrors.IsNotFound(env.Client(platform).Get(env.Ctx, client.ObjectKeyFromObject(cluster), cluster))
		}, 3).Should(BeTrue(), "Cluster should be deleted")

		// should create a new cluster
		req2 := &clustersv1alpha1.ClusterRequest{}
		Expect(env.Client(platform).Get(env.Ctx, ctrlutils.ObjectKey("no-delete", "foo"), req2)).To(Succeed())
		Expect(req2.Status.Cluster).To(BeNil())

		env.ShouldReconcile(schedulerRec, testutils.RequestFromObject(req2))
		Expect(env.Client(platform).Get(env.Ctx, client.ObjectKeyFromObject(req2), req2)).To(Succeed())
		Expect(req2.Status.Cluster).ToNot(BeNil())
		Expect(env.Client(platform).Get(env.Ctx, ctrlutils.ObjectKey(req2.Status.Cluster.Name, req2.Status.Cluster.Namespace), cluster)).To(Succeed())
		...
		Expect(env.Client(platform).Get(env.Ctx, client.ObjectKeyFromObject(cluster), cluster)).To(Succeed(), "Cluster should not be deleted")
	...
	It("should not consider clusters that are in deletion for scheduling", func() {
		...
		Expect(env.Client(platform).Get(env.Ctx, client.ObjectKeyFromObject(c), c)).To(Succeed())

		cr := &clustersv1alpha1.ClusterRequest{}
		cr.SetName("shared")
		cr.SetNamespace("foo")
		Expect(env.Client(platform).Get(env.Ctx, client.ObjectKeyFromObject(cr), cr)).To(Succeed())
		env.ShouldReconcile(schedulerRec, testutils.RequestFromObject(cr))
		Expect(env.Client(platform).Get(env.Ctx, client.ObjectKeyFromObject(cr), cr)).To(Succeed())
		...
		Expect(env.Client(platform).Update(env.Ctx, c)).To(Succeed())
		Expect(env.Client(platform).Delete(env.Ctx, c)).To(Succeed())
		Expect(env.Client(platform).Get(env.Ctx, client.ObjectKeyFromObject(c), c)).To(Succeed())
